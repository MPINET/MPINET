...
#include "utility.h"
#include "../net-jit/utility.h"
#include "../net-jit/dml-application.h"
#include "../net-jit/net-hit-application.h"
...
main(int argc, char** argv)
{
    ...
    */
    ...
    auto v = ns3::parse_DML_batch_traces(tracePath);
    auto trace = ns3::parse_net_jit_traces("../net-jit/traces/bert/bert-0.1.txt", std::move(v));
    DMLApplicationHelper dmlappHelper(trace, callback);

    NS_ASSERT_MSG(trace.size() <= fattree.servers.GetN(), "fattree size is not enough");
    ...
    for (ns3::MPIRankIDType rankID = 0; rankID < traces.size(); ++rankID) {
        ...
        dmlappHelper.install_node(node, rank, ip);
    }

    // rankID 0 is the master
    // install dmlapplication
    ns3::ApplicationContainer applications = dmlappHelper.install(0);
